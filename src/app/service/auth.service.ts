...
export class AuthService {
  ...
  createUser(user: User): Observable<any> {
    return this.http.post('http://localhost:4700/api/visitorData', user);
  }

  login(email: string, password: string): Observable<any> {
    this.authData.email = email;
    this.authData.password = password;
    return this.http.post<{ token: string, userId: string, expiresIn: any}>('http://localhost:4700/api/visitorData', this.authData).pipe(map(response => {
  ...
  getCurrentUser(userId: string): Observable<any> {
    ...
    return this.http.get('http://localhost:4700/api/visitorData', {params: httpParams});
  }
